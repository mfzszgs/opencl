...
//    PlatformId=(cl_platform_id*)alloca(sizeof(cl_platform_id)*2);
    
	err = clGetPlatformIDs(2, PlatformId, &numPlatforms);
    size_t size;
    err = clGetPlatformInfo(PlatformId[1],CL_PLATFORM_NAME,0,NULL,&size);
    char*name=(char*)alloca(sizeof(char)*size);
    err = clGetPlatformInfo(PlatformId[1],CL_PLATFORM_NAME,size,name,&size);
    cout<<name<<endl;

	err = clGetDeviceIDs(PlatformId[1], CL_DEVICE_TYPE_CPU, 1, &device_id, NULL);
	cl_context_properties properties[] =
	{
		CL_CONTEXT_PLATFORM, (cl_context_properties)PlatformId[1], 0
	};
